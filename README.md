...
| 分类管理 | 新增、编辑、删除、上架/下架、长按拖拽排序 |
| 批量操作 | 按分类和采购方式筛选后，一键上架或下架当前批次菜品 |
| 菜品排序 | 进入具体分类且未使用筛选条件时，长按菜品左侧拖拽排序 |
...
确认用户下单时允许订阅，模板 ID 与公众平台模板一致，模板字段仍为 `phrase1`、`time2`、`thing3`，并重新部署相关云函数。一次性订阅只能使用一次，测试时需要重新提交订单并再次授权。
