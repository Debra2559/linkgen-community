const { getContent, getPosts, savePosts, getProfile, currentMemberId, toggleInterest, toggleJoin, transitionTask } = require('../../utils/community-data');
Page({
  ...
  taskAction(e) { try { if (e.currentTarget.dataset.action === 'interest') toggleInterest(this.id, currentMemberId()); else toggleJoin(this.id, currentMemberId()); this.refresh(); } catch (error) { wx.showToast({ title: error.message, icon: 'none' }); } },
  advanceTask(e) { try { transitionTask(this.id, e.currentTarget.dataset.status, currentMemberId()); this.refresh(); } catch (error) { wx.showToast({ title: error.message, icon: 'none' }); } },
