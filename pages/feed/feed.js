const { getPosts, savePosts, seedLocalData } = require('../../utils/linkgen-data');

Page({
  data: { posts: [], filteredPosts: [], search: '', activeTag: '全部', tags: ['全部', 'AI 工具', '产品思维', '独立开发', '个人成长'] },
  onLoad() { seedLocalData(); this.refresh(); },
  onShow() { this.refresh(); },
  onPullDownRefresh() { this.refresh(); wx.stopPullDownRefresh(); },
  refresh() { const posts = getPosts(); this.setData({ posts }, this.filterPosts); },
  filterPosts() { const { posts, search, activeTag } = this.data; const key = search.trim().toLowerCase(); const list = posts.filter((post) => (!key || `${post.title}${post.content}${post.author}${post.tags.join('')}`.toLowerCase().includes(key)) && (!activeTag || activeTag === '全部' || post.tags.includes(activeTag))); this.setData({ filteredPosts: list }); },
